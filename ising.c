...
#define D 5/*100 seems the upper limit to reasonable processing time for serial computing.*/
#define UP 1
#define DOWN -1
...
static void write_specific_heat_to_file(double* specific_heat_array, double* temp_array){
    ...
    for(int i = 0; i<(TEMP_MAX/TEMP_INTERVAL); i++){
            fprintf(fc,"%lg\t%lg\n",temp_array[i]/T_0,specific_heat_array[i]/(D*D*K_B));
        }
    fclose(fc);
}
...
static void write_magnetism_to_file(double* magnetism_array, double* temp_array){
    ...
    for(int i = 0; i<(TEMP_MAX/TEMP_INTERVAL); i++){
            fprintf(fc,"%lg\t%lg\n",temp_array[i]/T_0,magnetism_array[i]/(MU0*D*D));
        }
    fclose(fc);
}
...
static double store_consecutive_total_energy_calc_average(double* array_of_consecutive_total_energy, double total_energy, int iteration){
    ...
    if(STDEV_ARRAY_LENGTH-1 == modulo_element){/*If the array has filled up*/
       ...
       double total_energy_av = total_energy_sum/STDEV_ARRAY_LENGTH;/*store average value*/
...
static double calculate_magnetisation(int* spin_array){
    ...
    printf("TEST: TOTAL SPIN: %f\n",total_spin);
    double Magnetisation = total_spin/(D*D);
    printf("MAGE TEST: %lf\n",Magnetisation);
...
static double* evolve_system_random_spin_beta(int* spin_array, VAR_TYPE rng_x_coord, VAR_TYPE rng_y_coord, VAR_TYPE rng_Metropolis_U, double Temp){
    ...
    double *ret_vals = (double*)malloc(2*sizeof(double));
